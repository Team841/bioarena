...
echo ""
echo "deploy-pi.sh (Git Bash) and deploy-pi.ps1 (PowerShell) do all of the below in one"
echo "command:"
echo "       ./deploy-pi.sh                    # code change only, copies one file"
echo "       ./deploy-pi.sh --assets --service # when those changed too"
echo ""
...
echo "       chmod +x /opt/bioarena/$OUTPUT"
echo "     Leave the copied files owned by <USER>. The service only reads them, and only"
echo "     writes event.db, logs/ and db/backups/, which it creates and so already owns."
echo "     Chowning them to bioarena breaks the next deploy: scp overwrites by opening the"
echo "     existing file for writing, which a group-readable file owned by another account"
echo "     refuses regardless of the directory's permissions."
echo ""
...
echo "       chmod +x /opt/estop-panel/estop-panel   # on the panel Pi; leave ownership alone"
echo ""
