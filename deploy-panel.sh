...
fail() {
	...
	echo "" >&2
	echo "Nothing was started, so the panel is running whatever it was running before." >&2
	echo "Fix the cause and run this again -- repeating it is safe." >&2
	exit 1
}

trap 'fail "step $step did not complete"; rm -f "$SERVICE_FILE"' ERR

announce "Building for the $ALLIANCE panel"
GOOS=linux GOARCH=arm64 go build -o estop-panel-pi ./cmd/estop-panel
echo "      estop-panel-pi built"

announce "Setting the $ALLIANCE panel address to $PANEL_ADDRESS"
...
echo "      service file prepared"

announce "Checking $TARGET is reachable"
ssh -o ConnectTimeout=10 "$REMOTE" "true"
echo "      $REMOTE answers"

announce "Preparing /opt/estop-panel"
# The gpio group is the one that bites: without it the panel starts, reports that it
# cannot open the GPIO chip, and then reports no stops at all -- a field that looks healthy
# with e-stops that do nothing.
ssh "$REMOTE" "
	set -e
	...
	sudo mkdir -p /opt/estop-panel $STAGING
	sudo chown bioarena:bioarena /opt/estop-panel
"
echo "      service account, gpio access, and directory ready"

announce "Copying and installing"
ssh "$REMOTE" "sudo systemctl stop estop-panel 2>/dev/null || true"
scp -q estop-panel-pi "$REMOTE:$STAGING/estop-panel"
scp -q estop-panel.yaml "$REMOTE:$STAGING/" 2>/dev/null || true
...
ssh "$REMOTE" "
	...
	[ -f $STAGING/estop-panel.yaml ] && sudo install -o bioarena -g bioarena -m 644 $STAGING/estop-panel.yaml /opt/estop-panel/estop-panel.yaml || true
...
announce "Starting the $ALLIANCE panel"
ssh "$REMOTE" "sudo systemctl start estop-panel"
sleep 2
if ! ssh "$REMOTE" "systemctl is-active --quiet estop-panel"; then
	echo "" >&2
	echo "DEPLOY FAILED: the panel service did not stay running." >&2
	echo "" >&2
	ssh "$REMOTE" "sudo journalctl -u estop-panel -n 20 --no-pager" >&2 || true
	exit 1
fi
...
# Spelled out rather than capitalised with ${ALLIANCE^}, which needs bash 4 and would fail
# on the bash macOS ships.
